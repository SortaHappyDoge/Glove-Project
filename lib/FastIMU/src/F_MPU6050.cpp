...
int MPU6050::init(calData cal, uint8_t address) 
{
	...
	writeByte(IMUAddress, MPU6050_INT_PIN_CFG, 0x22);	// 00100010
...
void MPU6050::getRaw(RawData* raw){
	uint8_t rawData[14];
	int16_t IMUCount[7];
	readBytes(IMUAddress, MPU6050_ACCEL_XOUT_H, 14, &rawData[0]);

	IMUCount[0] = ((int16_t)rawData[0] << 8) | rawData[1];
	IMUCount[1] = ((int16_t)rawData[2] << 8) | rawData[3];
	IMUCount[2] = ((int16_t)rawData[4] << 8) | rawData[5];
	IMUCount[3] = ((int16_t)rawData[6] << 8) | rawData[7];
	IMUCount[4] = ((int16_t)rawData[8] << 8) | rawData[9];
	...
	raw->accelX = IMUCount[0];
	raw->accelY = IMUCount[1];
	raw->accelZ = IMUCount[2];
	raw->gyroX = IMUCount[4];
	raw->gyroY = IMUCount[5];
	raw->gyroZ = IMUCount[6];
}
